...
w = 1;
for i = 1:genotypeFactor.card(1)
  for j = 1:genotypeFactor.card(2)
    for k = 1:genotypeFactor.card(3)
      p1 = genotypesToAlleles(i,:);
      p2 = genotypesToAlleles(j,:);
      c = genotypesToAlleles(k,:);
      
      c = getChildAllele(c);
      par = getChildAlleles(p1,p2);
      m = getMatchCount(c, par);
      t = rows(par);
      
      p = m/t;
      genotypeFactor.val(w) = p;
...
function childAlleles = getChildAlleles(p1, p2)
  childAlleles = allcomb(p1,p2);
endfunction;

function childAllele = getChildAllele(c)
  if all(c == c(1))
    childAllele = c;
  else
    childAllele = [c(1),c(2);c(2),c(1)];
  endif;
endfunction;


function countMatch = getMatchCount(a, b)
  if all(a == a(1)) || rows(a) > 1
    countMatch = sum(ismember(b, a, 'rows'));
  else
    countMatch = sum(ismember(b, a, 'rows')) + sum(ismember(b, [a(2),a(1)], 'rows'));
  endif;
endfunction;
