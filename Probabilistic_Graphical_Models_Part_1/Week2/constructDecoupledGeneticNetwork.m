...
for i=1:numPeople
  parItem = pedigree.parents(i,:);
  if (all(parItem == 0))
    factorList(i) = childCopyGivenFreqsFactor(alleleFreqs, i);
  else
    factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, parItem(1), parItem(1)+numPeople);
  endif;
end;
for i=numPeople+1:2*numPeople
  parItem = pedigree.parents(i-numPeople,:);
  if (all(parItem == 0))
    factorList(i) = childCopyGivenFreqsFactor(alleleFreqs, i);
  else
    factorList(i) = childCopyGivenParentalsFactor(numAlleles, i, parItem(2), parItem(2)+numPeople);
  endif;
end;

for i=2*numPeople+1:3*numPeople
  parItem = pedigree.parents(i - 2*numPeople,:);
